...
import com.onyx.android.dr.reader.requests.DictionaryQueryRequest;
...
public class DictionaryQueryAction {
    ...
    public void execute(final ReaderPresenter readerPresenter, final BaseCallback callback) {
        final DictionaryQueryRequest resolverQueryRequest = new DictionaryQueryRequest(readerPresenter,
                token);
        readerPresenter.getReader().submitRequest(readerPresenter.getReaderView().getViewContext(),
                resolverQueryRequest, new BaseCallback() {
                    @Override
