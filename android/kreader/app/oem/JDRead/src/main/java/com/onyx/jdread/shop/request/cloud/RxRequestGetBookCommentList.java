...
public class RxRequestGetBookCommentList extends RxBaseCloudRequest {
    ...
    private void checkQuestResult() {
        if (bookCommentsResultBean != null && bookCommentsResultBean.result_code != 0) {
            ShopDataBundle.getInstance().getEventBus().post(new RequestFailedEvent(bookCommentsResultBean.message));
        }
    }
