...
//}
//
//P1363_API int	RAND_CSPRNG(csprng *RNG)
//{
...
//#define out_log(x, xlen) {\
//	octet oTest;\
//	OCTET_INIT(&oTest, xlen);\
//	memcpy(oTest.val, x, xlen);\
//	oTest.len = xlen;\
//	OCTET_OUTPUT(&oTest);\
//	OCTET_KILL(&oTest);\
//	printf("\n");}
...
P1363_API BOOL AES_CBC_IV0_DECRYPT(octet *k,octet *c,FILE *ifp,octet *m,FILE *ofp)
{
	...
	if (c)
	{
		if (m)
		{
			assert(m->max >= c->len - 16);
			m->len = aes_decrypt(&a, c->val, c->len, m->val, TRUE);
			if (-1 == m->len)
				return FALSE;
			...
			if (ofp)
			{
				...
				fwrite(m->val, m->len, 1, ofp);
			};
		}
		else
		{
			octet pTempBuf;
			OCTET_INIT(&pTempBuf, c->len);
			pTempBuf.len = aes_decrypt(&a, c->val, c->len, pTempBuf.val, TRUE);
			if (-1 == pTempBuf.len)
				return FALSE;

			assert(ofp);
			fseek(ofp, 0, SEEK_SET);
			fwrite(pTempBuf.val, pTempBuf.len, 1, ofp);
		}
...
//		char ibuf[nVirtualBlockSize], obuf[nVirtualBlockSize];
		char *ibuf = (char*)malloc(nVirtualBlockSize);
		char *obuf = (char*)malloc(nVirtualBlockSize);

		fseek(ifp, 0, SEEK_END);
		nInputFileLen = ftell(ifp);
		nVirtualBlocks = nInputFileLen / nVirtualBlockSize;
		...
		if (m)
		{
			assert(m->max >= nInputFileLen - 16);
			m->len = 0;
			ptr = m->val;
		};

		nLastBlockLen = nInputFileLen % nVirtualBlockSize;
		if (!nLastBlockLen)
		{
			--nVirtualBlocks;
...
// 			out_log(a.f, 16);
			...
			if (ofp)
				fwrite(obuf, nVirtualBlockSize, 1, ofp);

			if (m)
			{
				memcpy(ptr, obuf, nVirtualBlockSize);
				ptr += nVirtualBlockSize;
				m->len += nVirtualBlockSize;
			};
		};
...
// 		out_log(a.f, 16);
		...
		if (ofp)
			fwrite(obuf, nLastLen, 1, ofp);
		if (m)
		{
			memcpy(ptr, obuf, nLastLen);
			m->len += nLastLen;
		};
...
P1363_API BOOL AES_DECRYPT_STRING(octet *k,octet *c,char* pInput,int nInputLen,octet *m,char** pOutput, int* nOutputLen, BOOL bPadding)
{
    aes a;
...
// 		return FALSE;
	...
	if (c)
	{
		...
		else
		{
			...
			strncpy(*pOutput, pTempBuf.val, pTempBuf.len);
		}

		return TRUE;
	};

	*pOutput = (char*)malloc(nInputLen+16);
	memset(*pOutput, 0, nInputLen+16);
	*nOutputLen = aes_decrypt(&a, pInput, nInputLen, *pOutput, bPadding);
...
{ /* AES CBC encryption, with Null IV */
	...
	if (m)
	{
		int nLeastOutLen = (m->len + 15) & 0xFFFFFFF0;
		if (c)
		{
			assert(c->max >= nLeastOutLen);
			c->len = aes_encrypt(&a, m->val, m->len, c->val, TRUE);
			if (-1 == c->len)
				return FALSE;
			...
			{
				fseek(ofp, 0, SEEK_SET);
				fwrite(c->val, c->len, 1, ofp);
			};
		}
		else
		{
			octet pTempBuf;
			OCTET_INIT(&pTempBuf, nLeastOutLen);
			pTempBuf.len = aes_encrypt(&a, m->val, m->len, pTempBuf.val, TRUE);
			if (-1 == pTempBuf.len)
				return FALSE;
...
//		char ibuf[nVirtualBlockSize], obuf[nVirtualBlockSize];
		char *ibuf = (char*)malloc(nVirtualBlockSize);
		char *obuf = (char*)malloc(nVirtualBlockSize+16);

		fseek(ifp, 0, SEEK_END);
		nInputFileLen = ftell(ifp);
		nVirtualBlocks = nInputFileLen / nVirtualBlockSize;
		nLeastOutLen = (nInputFileLen + 15) & 0xFFFFFFF0;
		fseek(ifp, 0, SEEK_SET);
		if (ofp)
			fseek(ofp, 0, SEEK_SET);
		...
		for (i = nVirtualBlocks; i > 0; --i)
		{
			fread(ibuf, nVirtualBlockSize, 1, ifp);
// 			out_log(a.f, 16);
			aes_encrypt(&a, ibuf, nVirtualBlockSize, obuf, FALSE);
			...
			if (c)
			{
				memcpy(ptr, obuf, nVirtualBlockSize);
				ptr += nVirtualBlockSize;
				c->len += nVirtualBlockSize;
			};
		};
		fread(ibuf, nLastBlockLen, 1, ifp);
// 		out_log(a.f, 16);
		nLastLen = aes_encrypt(&a, ibuf, nLastBlockLen, obuf, TRUE);
		if(nLastLen < 0)
		{
			free(obuf);
			free(ibuf);
...
P1363_API BOOL AES_ENCRYPT_STRING(octet *k,octet *m,char* pInput,int nInputLen,octet *c,char** pOutput,int* nOutputLen, BOOL bPadding)
{ 
	aes a;
	...
	*pOutput = (char*)malloc(nInputLen+16);
	memset(*pOutput, 0, nInputLen+16);
	*nOutputLen = aes_encrypt(&a, pInput, nInputLen, *pOutput, bPadding);

	return TRUE;
}

P1363_API BOOL file_AES_encrypt(char* szKey, int nKeyLen, char* szInput, char* szOutput)
{
	FILE *pInput;
	FILE *pOutput;
	BOOL bRet;
	...
	octet KM, K;
	if( (NULL == szKey ) || (0 == nKeyLen) || (NULL == szInput) || (NULL == szOutput))
		return FALSE;
	...
	bRet = AES_CBC_IV0_ENCRYPT(&K, 0, pInput, 0, pOutput);
...
P1363_API BOOL file_AES_decrypt(char* szKey, int nKeyLen, char* szInput, char* szOutput)
{
	FILE *pInput;
	FILE *pOutput;
	BOOL bRet;
	...
	bRet = AES_CBC_IV0_DECRYPT(&K, 0, pInput, 0, pOutput);
...
P1363_API BOOL string_AES_encrypt(char* szKey, char* szInput, int nInputLen, char** szOutput, int* nOutputLen, BOOL bPadding)
{
	int nKeyLen;
	BOOL bRet;
	octet KM, K;

	if( (NULL == szKey) || (NULL == szInput) || (NULL == szOutput) )
		return FALSE;
	...
	bRet = AES_ENCRYPT_STRING(&K, 0, szInput, nInputLen, 0, szOutput, nOutputLen, bPadding);
...
P1363_API BOOL string_AES_decrypt( char* szKey, char* szInput, int nInputLen, char** szOutput, int* nOutputLen, BOOL bPadding )
{
	int nKeyLen;
	octet KM, K;
	BOOL bRet;
	...
	HASH(SHA256, &KM, &K);
	K.len = 32;
	bRet = AES_DECRYPT_STRING(&K, 0, szInput, nInputLen, 0, szOutput, nOutputLen, bPadding);
...
P1363_API void FreePtr_AES( void* ptr )
{
	if (NULL != ptr)
	{
		free(ptr);
...
#ifndef _CLIENT
P1363_API int GenerateAESKey(char** ppKey)
{
	char raw[256] = {0};
	char cseed[256] = {0};
	char seeds[512] = {0};
	...
	sprintf(raw, "%I64d", ((unsigned __int64)timehi << 32) + (unsigned __int64)timelo);
#else
	__asm__ ("rdtsc": "=a" (timelo),"=d" (timehi));

	sprintf(raw, "%lld", ((unsigned long long)timehi << 32) + (unsigned long long)timelo);
#endif //WIN32

	time(&seed);

#ifdef WIN32
	sprintf(cseed, "%I64d", (unsigned __int64)seed);
#else
	sprintf(cseed, "%lld", (unsigned long long)seed);
#endif //WIN32

	nseedslen = strlen(raw) + strlen(cseed);
	strcat(seeds, raw);
	strcat(seeds, cseed);

	Hash_256(seeds, nseedslen, ppKey, &nKeyLen);

	return 0;
}

#define RANDOM_NUM_LEN        16
P1363_API int GenerateRandomNum(char** pRandomNum, int* nRandomLen)
{
	char raw[256] = {0};
	char cseed[256] = {0};
	char seeds[512] = {0};
	...
	if((NULL == pRandomNum) || (nRandomLen <= 0))
		return -1;
	...
	Hash_256(seeds, nseedslen, &pRandom, &nLen);

	*nRandomLen = RANDOM_NUM_LEN;
	*pRandomNum = (char*)malloc(*nRandomLen+1);
	memset(*pRandomNum, 0, *nRandomLen+1);
	memcpy(*pRandomNum, pRandom, *nRandomLen);
...
static int HASH_256_LEN = 32;
P1363_API void Hash_256( char* pInputBuf, int nInputBufLen, char** pOutputBuf, int* nOutputBufLen )
{
	octet InBuf, OutBuf;

	OCTET_INIT(&InBuf, nInputBufLen);
	OCTET_INIT(&OutBuf, HASH_256_LEN);

	memcpy(InBuf.val, pInputBuf, nInputBufLen);
	InBuf.len = nInputBufLen;

	HASH(SHA256, &InBuf, &OutBuf);
	OutBuf.len = HASH_256_LEN;

	*pOutputBuf = (char*)malloc(HASH_256_LEN+1);
	memset(*pOutputBuf, 0, HASH_256_LEN+1);
	memcpy(*pOutputBuf, OutBuf.val, HASH_256_LEN);

	*nOutputBufLen = HASH_256_LEN;
...
//        sscanf(params[0],"%d\n",&bits); 
//
//    DOM->words=3+(bits/MIRACL);   /* increase the 3? - may be required... */
//        
