...
public class UnitCollection {
    ...
    public static List<String> getALLUnitNamesByCategory(@NonNull Context context, int category) {
        UnitCollection[] collections = getInstance(context);
        UnitCollection collection = collections[category];
        int size = collection.getItems().size();
        List<String> names = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            SingleUnit unit = collection.getItems().get(i);
            names.add(getUnitName(context, unit));
        }
        return names;
    }

    public static String getUnitName(@NonNull final Context context, final SingleUnit unit) {
        int id = context.getResources().getIdentifier(unit.getName(), "string", context.getPackageName());
        String name = "";
        try {
        ...
        return name.isEmpty() ? unit.getName() : name;
    }
