...
public class TTFScan {
	...
	public static int getUInt16Buff(byte[] b, int offset) {
		int 	res = 0;
		res = (b[offset++] & 0xff) << 8;
		res |= (b[offset] & 0xff);
		return res;
	...
	public static int getUInt32Buff(byte[] b, int offset) {
		int 	res = 0;
			...
			case 2:
				if (ttfi.Type == 0 || flag2) {					
					try {
						...
						if (s.indexOf("italic") >= 0 || s.indexOf("oblique") >= 0) {
							ttfi.Type += 2;
							flag2 = false;
						}
						if (s.indexOf("normal") >= 0 || s.indexOf("regular") >= 0) {						
							flag2 = false;
						}
					} catch (UnsupportedEncodingException e) {}					
				} 
