...
public final class TermuxInstaller {
    ...
    public static void setupIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        final File PREFIX_FILE = new File(Constant.PREFIX_PATH);
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    final String STAGING_PREFIX_PATH = Constant.FILES_PATH + "/usr-staging";
...
//                    final URL zipUrl = determineZipUrl();
                    try (ZipInputStream zipInput = new ZipInputStream(assetManager.open("onyx-bootstrap-arm.zip"))) {
                        ZipEntry zipEntry;
                            ...
                            if (zipEntry.getName().equals("SYMLINKS.txt")) {
                                ...
                                if (zipEntry.isDirectory()) {
                                    ...
                                    if (zipEntryName.startsWith("bin/")
                                            ...
                                            || zipEntryName.startsWith("lib/node_modules/npm")
                    ...
                    Log.e(Debug.LOG_TAG, "Bootstrap error", e);
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            try {
    ...
    public static void setupStorageSymlinks(final Context context) {
        ...
        new Thread() {
            public void run() {
                try {
                    File storageDir = new File(Constant.HOME_PATH, "storage");
