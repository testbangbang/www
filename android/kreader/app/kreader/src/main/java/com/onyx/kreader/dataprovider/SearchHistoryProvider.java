...
public class SearchHistoryProvider {
    ...
    public static final List<SearchHistory> getLatestSearchHistory(String md5, int count){
        return new Select().from(SearchHistory.class).where(SearchHistory_Table.md5.eq(md5))
                .limit(count)
    ...
    public static void deleteSearchHistory(String md5) {
        SQLite.delete(SearchHistory.class).where(SearchHistory_Table.md5.eq(md5)).query();
    }
