...
public abstract class AlFormat {
	...
	public final ArrayList<String> 			bookAuthors = new ArrayList<String>(0);
	public final ArrayList<String> 			bookGenres = new ArrayList<String>(0);
	public final ArrayList<String> 			bookSeries = new ArrayList<String>(0);
	public String			  				bookTitle = null;
	public AlFiles 							aFiles = null;
	public final ArrayList<AlOneSearchResult>	resfind = new ArrayList<AlOneSearchResult>(0);
	
	final ArrayList<AlOneParagraph>		par = new ArrayList<AlOneParagraph>(0);
	final ArrayList<AlOneImage>			im = new ArrayList<AlOneImage>(0);
	private final ArrayList<AlOneTable>			ta = new ArrayList<AlOneTable>(0);
	final ArrayList<AlOneLink>			lnk = new ArrayList<AlOneLink>(0);
	public final ArrayList<AlOneContent>		ttl = new ArrayList<AlOneContent>(0);
	...
	final StringBuilder				state_specialBuff0 = new StringBuilder();
	...
	private final ArrayList<AlOneStyleStack> styleStack = new ArrayList<AlOneStyleStack>(0);
	private int styleStack_point = 0;
	...
	int findParagraphByPos(int pos) {
		...
		if (pos < 0)
			pos = 0;
		return findParagraphByPos0(0, par.size(), pos);
	}

    /*private char[]  arabic_temp_data = null;
    private int     arabic_temp_data_len = 0;*/
    ...
    private final void reverseArabicWord(char[] data, char[] temp, int startAll, ArrayList<arabicWord> token) {
    ...
    /*private final void scanArabic(AlOneParagraph alp) {
        boolean isInvisible = false;
        int i, len = alp.length, word_start = -1, countArabic = 0, countNormal = 0, startArabic = 0, lastChar = 0;
        char ch;

        ArrayList<arabicWord> token = new ArrayList<arabicWord>();
        ...
        for (i = 0; i < len; i++) {
            ...
            if (ch == 0x20) {
                if (!isInvisible) {
                    if (word_start != -1) {
                        if (countArabic != 0 && countNormal == 0) {
                            if (token.size() > 0 || countArabic != 0) {
                                if (token.size() == 0)
                                    startArabic = word_start;
                        ...
                        if (countNormal != 0) {
                            if (token.size() > 1) {
                                if (arabic_temp_data == null || arabic_temp_data_len < len) {
                                    arabic_temp_data_len = len;
                                    arabic_temp_data = new char[len];
                                }
                                reverseArabicWord(stored_par.data, arabic_temp_data, startArabic, token);
                            }
                            token.clear();
                        }

                        word_start = -1;
                        countArabic = countNormal = 0;
                    }
                }
            } else {
                if (word_start == -1) {
	...
	private int getParagraphSlot(int pos, char[] slot_t, long[] slot_s, AlProfileOptions profiles) {
		...
		for (i = 0; i < j; i++) {
			...
			//	
			//} else		
			if (((style & AlStyles.SL_PAR) != 0) && (!isInvisible) &&
    ...
    public String getDictWordByPos(int posStart,int posEnd) {
        ...
        for (int j = 0; j < ap.length; j++) {
            ...
            if (isAccept) {
                ...
                if (AlUnicode.isApostrophe(ch) || ch == '-' || ch == '\'' || ch == 0x301){
                    if (ch != 0x301) {
                        if (res.length() > 0)
                            res.append(ch);
                    }
    ...
    public String getTextByPos(int posStart,int posEnd) {
        if (posStart < 0 | posEnd >= size)
            return null;
        if (posStart > posEnd)
            return null;
