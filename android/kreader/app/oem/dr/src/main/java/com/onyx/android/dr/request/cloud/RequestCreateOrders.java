...
public class RequestCreateOrders extends BaseCloudRequest {
    ...
    public RequestCreateOrders(ProductRequestBean product) {
    ...
    private void createOrders(CloudManager parent) {
        try {
            Response<ProductOrder<CloudMetadata>> response = executeCall(ServiceFactory.getContentService(parent.getCloudConf().getApiBase()).createOrders(product));
            if (response != null) {
                order = response.body();
            }
        } catch (Exception e) {
