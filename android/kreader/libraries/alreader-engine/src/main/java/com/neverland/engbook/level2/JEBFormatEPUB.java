...
public class JEBFormatEPUB extends AlFormatBaseHTML {
    ...
    private final ArrayList<JEBFormatEPUB.AlOneEPUBTOC>		listTOC = new ArrayList<>();
    ...
    private final HashMap<String, JEBFormatEPUB.AlOneEPUBIDItem>	mapId = new HashMap<>();
    private final ArrayList<JEBFormatEPUB.AlOneEPUBItem>			listFiles = new ArrayList<>();
    ...
    protected void prepareCustom() {
        if (listTOC.size() > 0) {
            ...
            for (int i = 0; i < listTOC.size(); i++) {
                JEBFormatEPUB.AlOneEPUBTOC a = listTOC.get(i);
    ...
    private void addFiles2Read(String fname, int sp) {
        String addFiles = AlFiles.getAbsoluteName(currentFile, fname);
        ((JEBFilesEPUB) aFiles).addFilesToRecord(addFiles, sp);
        if (dinamicSize) {
                    ...
                    if ((allState.description & AlStateLevel2.PAR_DESCRIPTION3) != 0) {
                        if (toc_state == TOC_NAVMAP + TOC_NAVPOINT) {
                            ...
                            if (toc_point.length() > 0 && toc_text.length() > 0) {
                                JEBFormatEPUB.AlOneEPUBTOC a = new JEBFormatEPUB.AlOneEPUBTOC();
                                a.href = toc_point;
            ...
            case AlFormatTag.TAG_ITEMREF:
                if (tag.closed) {
                    ...
                    if ((allState.description & AlStateLevel2.PAR_DESCRIPTION2) != 0) {
                        ...
                        if (param != null) {
                            JEBFormatEPUB.AlOneEPUBIDItem it = mapId.get(param.toString());
                            if (it != null) {
                                JEBFormatEPUB.AlOneEPUBItem a = new JEBFormatEPUB.AlOneEPUBItem();
                                a.file = it.href;
                                a.type = it.type;
                                listFiles.add(a);
                            }
                        ...
                        if (param != null) {
                            ...
                            if (href != null && type != null) {
                                ...
                                if (aFiles.getExternalFileNum(file) != AlFiles.LEVEL1_FILE_NOT_FOUND) {
                                    JEBFormatEPUB.AlOneEPUBIDItem a = new JEBFormatEPUB.AlOneEPUBIDItem();
                                    a.href = file;
            ...
            case AlFormatTag.TAG_EXTFILE:
                if (tag.closed) {
                    ...
                    if ((allState.description & AlStateLevel2.PAR_DESCRIPTION2) != 0) {
                        ...
                        if (coverMETAIID != null) {
                            JEBFormatEPUB.AlOneEPUBIDItem it = mapId.get(coverMETAIID);
                            coverMETAIID = null;
                            if (it != null)
    ...
    private void getBlockSize(){
        if(fileNum != active_file){
            fileNum = active_file;
            blockSize = ((JEBFilesEPUB) aFiles).getRecordFileSize(fileNum);
        }
    }
