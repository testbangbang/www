...
/** 
 *  \mainpage Reader Mobile 10.0 SDK Embedding API
 *
 ...
 * The Location returned is put in the place specified by the result parameter. No commands that
 * access the Document or the Renderer may be called between the getContentIterator command that
 * created the content iterator and this command.
 * 
 ...
 * the flags parameter are passed through the API as the flags. No commands that access the Document
 * or the Renderer may be called between the getContentIterator command that created the content
 * iterator and this command. See notes for abortCount parameter description.
 * 
 ...
 * 
 * \commanddef{getRangeInfoBoxFlags, index flags}
 *
 * \commandparams
 * \commandparam{index,input,integer}
 * \commandparam{flags,output,integer}
 *
 * Executes the dpdoc::RangeInfo::getBoxFlags API on the currently saved range info. The index specifies the box in case more than one box is needed to cover the range. No commands that access the Renderer may be called between the getRangeInfo command that created the range and this command.
 *
 ...
 * parameter.
 *
 * \commanddef{licenseConsume, type, count}
 *
 * \commandparams
 * \commandparam{type,input,string}
 ...
 * 
 * \commanddef{renderRectRaw, format xMin yMin xMax yMax abortCount}
 *
 * \commandparams
 * \commandparam{format,input,string}
 * \commandparam{xMin,input,integer}
 ...
 * \commandparam{abortCount,input,integer}
 *
 * Executes the dpdoc::Renderer::paint API. It is similar to the renderRect command, but instead of writing out a PNG file, it writes out the raw image data to a .raw file.
 *
 ...
 * 
 * \commanddef{closeCurrentDocument}
 *
 * \commandparams
 * none
 *  
 * Closes the current document. Usefull to track idle memory state
 *
 ...
 * 
 * 
 * \commanddef{setReturnEpubCfiBookmarks, loc mode}
 *
 * \commandparams
 * \commandparam{loc,input,locaiton}
 * \commandparam{mode,input,integer}
 *  
 * Executes the dpdoc::Location::setReturnEpubCfiBookmarks API passing value from the mode parameter as the boolean value (!=0).
 *
