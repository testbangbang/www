...
unsigned const char * RyoGothicPlusN_Medium()
{
    static unsigned const char * data = 0;
    if (data == 0)
    {
#ifdef WIN32
        wchar_t * absPathName = L"C:\\Users\\Li\\Desktop\\adobefont\\RyoGothicPlusN_Medium_FONT";
        ...
        char * p = font_path;
        p += strlen(path);
        strcpy(p, "RyoGothicPlusN_Medium_FONT.ttf");
        printf("Font Path:%s\n\n", font_path);
        ...
        if( file >= 0 )
        {
            ...
            if( fstat( file, &sbuf ) == 0 )
            {
                ...
                printf("RyoGothicPlusN_Medium_FONT Size:%d\n\n", size);
...
unsigned const int RyoGothicPlusN_Medium_Length()
{
    static unsigned int length = 0;
    if (length == 0)
    {
#ifdef WIN32
        wchar_t * absPathName = L"C:\\Users\\Li\\Desktop\\adobefont\\RyoGothicPlusN_Medium_FONT.ttf";
        void * file = CreateFileW( absPathName, GENERIC_READ, FILE_SHARE_READ, NULL, 
                                     OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
        ...
        if( file >= 0 )
        {
            ...
            if( fstat( file, &sbuf ) == 0 )
            {
                ...
                printf("RyoGothicPlusN_Medium_Length:%d\n\n", length);
            }
            close( file );
        }
#endif
...
//	#include "RyoGothicPlusN-Medium.h"
//#else
//	0
//#endif
//};
