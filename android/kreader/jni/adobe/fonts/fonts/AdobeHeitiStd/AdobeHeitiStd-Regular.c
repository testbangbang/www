...
unsigned const char * AdobeHeitiStd_Regular()
{
    static unsigned const char * data = 0;
    if (data == 0)
    {
#ifdef WIN32
        wchar_t * absPathName = L"C:\\Users\\Li\\Desktop\\adobefont\\AdobeHeitiStd_Regular_FONT.ttf";
        ...
        char * p = font_path;
        p += strlen(path);
        strcpy(p, "AdobeHeitiStd_Regular_FONT.ttf");
        printf("Font Path:%s\n", font_path);
        ...
        if( file >= 0 )
        {
            ...
            if( fstat( file, &sbuf ) == 0 )
            {
                ...
                printf("AdobeHeitiStd_Regular_FONT Size:%d\n", size);
...
unsigned const int AdobeHeitiStd_Regular_Length()
{
    static unsigned int length = 0;
    if (length == 0)
    {
#ifdef WIN32
        wchar_t * absPathName = L"C:\\Users\\Li\\Desktop\\adobefont\\AdobeHeitiStd_Regular_FONT";
        void * file = CreateFileW( absPathName, GENERIC_READ, FILE_SHARE_READ, NULL, 
                                     OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL );
        ...
        if( file >= 0 )
        {
            printf("Retrieve Adobe Heiti Length\n");
            struct stat sbuf;
            if( fstat( file, &sbuf ) == 0 )
            {
                length = (size_t)sbuf.st_size;
                printf("AdobeHeitiStd_Regular_Length:%d\n", length);
            }
            close( file );
        }
#endif
...
//	#include "AdobeHeitiStd-Regular.h"
//#else
//	0
//#endif
//};
