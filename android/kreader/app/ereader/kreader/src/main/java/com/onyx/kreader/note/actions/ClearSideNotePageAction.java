...
import com.onyx.kreader.note.request.ClearSideNotePageRequest;
...
public class ClearSideNotePageAction extends BaseAction {

    private List<PageInfo> pageInfo = new ArrayList<>();

    public ClearSideNotePageAction(final List<PageInfo> pages) {
        pageInfo.addAll(pages);
    }

    @Override
    public void execute(final ReaderDataHolder readerDataHolder, final BaseCallback baseCallback) {
        final ClearSideNotePageRequest clearPageRequest = new ClearSideNotePageRequest(pageInfo);
