...
/* define this for a quick space/time performance boost */
#define LOTS_OF_MEMORY 
...
{ /* Key=nk bytes */
  ...
  /* nr is number of rounds */
	...
	rijndaelKeySetupEnc(a->fkey, key, nk * 32);
	rijndaelKeySetupDec(a->rkey, key, nk * 32);
...
int aes_encrypt(aes *pCtrl, char *pInput, int nLen, char *pOutput, BOOL bPadding)
{
	int i, numBlocks, padLen;
	u8 block[16], *iv;

	//ctrl structure loss, error
	if (!pCtrl)
		return -1;
	...
	case MR_ECB:
		for (i = numBlocks; i > 0; i--) {
			rijndaelEncrypt(pCtrl->fkey, pCtrl->Nr, pInput, pOutput);
			pInput += 16;
			pOutput += 16;
		};
		...
		if (bPadding)
		{
			padLen = 16 - (nLen - 16*numBlocks);
			assert(padLen > 0 && padLen <= 16);
			memcpy(block, pInput, 16 - padLen);
			memset(block + 16 - padLen, padLen, padLen);

			rijndaelEncrypt(pCtrl->fkey, pCtrl->Nr, block, pOutput);
		};
		break;
	...
	case MR_CBC:
		iv = pCtrl->f;
		for (i = numBlocks; i > 0; i--) {
			((u32*)block)[0] = ((u32*)pInput)[0] ^ ((u32*)iv)[0];
			((u32*)block)[1] = ((u32*)pInput)[1] ^ ((u32*)iv)[1];
			((u32*)block)[2] = ((u32*)pInput)[2] ^ ((u32*)iv)[2];
			((u32*)block)[3] = ((u32*)pInput)[3] ^ ((u32*)iv)[3];
			rijndaelEncrypt(pCtrl->fkey, pCtrl->Nr, block, pOutput);
			iv = pOutput;
			pInput += 16;
			pOutput += 16;
	...
	return bPadding ? 16*(numBlocks + 1) : 16 * numBlocks;
}

int aes_decrypt(aes *pCtrl, char *pInput, int nLen, char *pOutput, BOOL bPadding)
{
	int i, numBlocks, padLen;
	u8 block[16], *iv;
	...
	case MR_ECB:
		for (i = numBlocks - 1; i > 0; --i) {
			rijndaelDecrypt(pCtrl->rkey, pCtrl->Nr, pInput, pOutput);
			pInput += 16;
			pOutput += 16;
		};

		rijndaelDecrypt(pCtrl->rkey, pCtrl->Nr, pInput, block);
		...
		if (bPadding)
		{
			...
			memcpy(pOutput, block, 16 - padLen);
		}
		else
		{
			memcpy(pOutput, block, 16);
		};
	...
	return bPadding ? 16*numBlocks - padLen : 16 * numBlocks;
}
