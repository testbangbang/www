 ...
 */
class HighlightView {
    static final String TAG = HighlightView.class.getSimpleName();
    ...
    public static final int MOVE_SPLIT_POINT = (1 << 6);
    ...
    public void setSplitPoint(float x, float y) {
        if (isHorizontalSplit()) {
            currentSplitPoint.x = x;
        }
        if (isVerticalSplit()) {
            currentSplitPoint.y = y;
        }
        alignSplitPoints(currentSplitPoint, pointRow, pointColumn);
        viewContext.invalidate();
    }

    public void setMotionEdge(int motionEdge) {
    ...
    public void setup(Matrix m, Rect imageRect, RectF cropRect, boolean maintainAspectRatio, final CropArgs args) {
        matrix = new Matrix(m);

        this.cropRect = cropRect;
        this.imageRect = new RectF(imageRect);
        this.maintainAspectRatio = maintainAspectRatio;
        this.isCropPage = args.manualCropPage();
        this.isSplitPage = args.manualSplitPage();
    ...
    protected void draw(Canvas canvas) {
        ...
        if (isSplitPage) {
            drawSplitPointHandleAndLine(canvas);
        }
    }
    ...
    private void drawSplitPointHandleAndLine(Canvas canvas) {
        outlinePaint.setColor(Color.BLACK);
        for (int row = 0; row < pointMatrix.rows(); row++) {
            for (int col = 0; col < pointMatrix.cols(); col++) {
                ...
                if (isVerticalSplit()) {
                    canvas.drawLine(drawRect.left, p.y, drawRect.right, p.y, outlinePaint);
                }
                if (isHorizontalSplit()) {
                    canvas.drawLine(p.x, drawRect.top, p.x, drawRect.bottom, outlinePaint);
                }
            }
        }
    }
    ...
    public int getHit(float x, float y) {
        ...
        final float splitPointHysteresis = 75F;
        ...
        if (isCropPage) {
            ...
            if (isSplitPage){
                currentSplitPoint = hitSplitPoints(x, y, splitPointHysteresis);
                if (currentSplitPoint != null) {
                    retval = MOVE_SPLIT_POINT;
                    return retval;
                }
            }
    ...
    void handleMotion(int edge, float x, float y, float dx, float dy,boolean bothway_stretch) {
        Rect r = computeLayout();
        if (edge == MOVE) {
            // Convert to image space before sending to moveBy()
            moveBy(dx * (cropRect.width() / r.width()),
                    dy * (cropRect.height() / r.height()));
        } else if (edge == MOVE_SPLIT_POINT) {
            ...
            growBy((((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * xDelta,
                    (((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * yDelta,bothway_stretch);

            resetSplitPoints();
        }
    }

    private int getSubScreenRows() {
        return rows;
    }

    private int getSubScreenColumns() {
        return columns;
    }
    ...
    private PointF hitSplitPoints(float x, float y, float hysteresis) {
        RefValue<Integer> row = new RefValue<Integer>(-1);
        RefValue<Integer> col = new RefValue<Integer>(-1);
        RefValue<PointF> point = new RefValue<PointF>(null);
        if (pointMatrix.hitTest(x, y, hysteresis, row, col, point)) {
            pointRow = row.getValue();
            pointColumn = col.getValue();
    ...
    private void resetSplitPoints() {
        ...
        pointMatrix.distribute(getSubScreenRows(), getSubScreenColumns(), drawRect.left, drawRect.top, drawRect.right, drawRect.bottom);
    }

    private void moveSplitPoints(float dx, float dy) {
        if (!isSplitPage) {
            return;
        }
        pointMatrix.offset(dx, dy);
    }

    private void alignSplitPoints(PointF point, int pointRow, int pointColumn) {
        for (int row = 0; row < pointMatrix.rows(); row++) {
            pointMatrix.get(row, pointColumn).x = point.x;
        }
        for (int col = 0; col < pointMatrix.cols(); col++) {
            pointMatrix.get(pointRow, col).y = point.y;
        }
    }
    ...
    // Grows the cropping rectangle by (dx, dy) in image space.
    void growBy(float dx, float dy,boolean bothway_stretch) {
        ...
        if (bothway_stretch) {
            ...
            if ((motionEdge & GROW_RIGHT_EDGE) == 0) {
                r.left -= dx;
            }
            if ((motionEdge & GROW_LEFT_EDGE) == 0) {
                r.right += dx;
            }
            if ((motionEdge & GROW_BOTTOM_EDGE) == 0) {
                r.top -= dy;
            }
            if ((motionEdge & GROW_TOP_EDGE) == 0) {
                r.bottom += dy;
            }
        }
