...
public class RxGetRechargeStatusRequest extends RxBaseCloudRequest {
    ...
    private void checkResult() {
        if (rechargeStatusBean != null && rechargeStatusBean.result_code != 0) {
            PersonalDataBundle.getInstance().getEventBus().post(new RequestFailedEvent(rechargeStatusBean.message));
        }
    }
