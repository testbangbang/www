...
public abstract class AlFormat {
	...
	public AlOneTable getTableByName(String name) {
		...
		for (i = 0; i < ta.size(); i++) {
			if (ta.get(i).positionS == table_start)
				return ta.get(i);
		}
    ...
    public String getTableSource(int address) {
        ...
        for (i = a.positionS; i < a.positionE;) {
            buf_cnt = AlFiles.LEVEL1_FILE_BUF_SIZE;
            if (i + buf_cnt > a.positionE) {
                buf_cnt = aFiles.getByteBuffer(i, parser_inBuff, a.positionE - i + 2);
                if (buf_cnt > a.positionE - i)
                    buf_cnt = a.positionE - i;
            } else {
