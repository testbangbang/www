...
#include <libpdfium/third_party/zlib_v128/zlib.h>
...
{
    drmCallback.isEncrypted = reinterpret_cast<FPDF_BOOL (*)()>(&isEncryptedStatic);

    typedef unsigned char* (*FUNC)(const unsigned char *data,
                                   const size_t dataLen,
                                   size_t *resultLen);
    drmCallback.decrypt = reinterpret_cast<FUNC>(&aesDecryptStatic);
}
...
IPDF_ONYX_DRMCALLBACK &DrmDecryptManager::getDrmCallback()
{
    return drmCallback;
}

bool DrmDecryptManager::isEncryptedStatic()
{
    return singleton().isEncrypted();
}

unsigned char *DrmDecryptManager::aesDecryptStatic(const unsigned char *data, const size_t dataLen, size_t *resultLen)
{
    return singleton().aesDecrypt(data, dataLen, resultLen);
}
