...
void setDecryptInfo(const char* bookKey,int bookKeyLen,const char* uuid,const char* rand){
	if(pKey != NULL){
		free(pKey);
		pKey = NULL;
	}
	...
	GetContentKeyBuf(bookKey, bookKeyLen, uuid, rand, &pKey, &keyLen);
}
...
bool DecryptData(unsigned char* pInputData, int inputLen, unsigned char** pOutData, int* outLen) {
 ...
 *pOutData = new unsigned char[inputLen + 1];
 unsigned char* ppOutData = *pOutData;
 int decryptOutLen = 0;
 ...
 for (int i = 0; i<nLoop; i++)
 {
  ...
  memcpy(inbuffer, pInputData + i*INBUFFER, INBUFFER);
 ...
 int finalInbufferlen = inputLen - nLoop*INBUFFER;
 unsigned char* finalInbuffer = new unsigned char[finalInbufferlen + 1];
 memset(finalInbuffer, 0, finalInbufferlen + 1);
 memcpy(finalInbuffer, pInputData + nLoop*INBUFFER, finalInbufferlen);
 ...
 memcpy(ppOutData + countDecryptLen, finalOutbuffer, finalOutbufferlen);
...
int GenerateRandom( char** ppRandomCipher, int* pRandomCipherLen )
{
	...
	{
		CLOG log;
		log.Write("GenerateRandom GenerateRandomNumʧ��! nRet=%d", nRet);
		log.Write("File %s, Line %d", __FILE__, __LINE__);
...
int GetContentKeyBuf(const char* pRightFileBuf, int nRightFileBuflen,const char* pDevIDHashCipher,const char* pRandomCipher, char** ppKey, int* nKeyLen)
{
	...
	char *pinDevID= NULL;
	if(pDevIDHashCipher != NULL){
		pinDevID = new char[strlen(pDevIDHashCipher)+1];
		strcpy(pinDevID, pDevIDHashCipher);
	}
	char *pinRandom = NULL;
	if(pRandomCipher != NULL){
       pinRandom = new char[strlen(pRandomCipher) + 1];
       strcpy(pinRandom,pRandomCipher);
	}
	...
	char* pRight = NULL;
	int nRightLen = 0;
	int nRet = AnalyticRightFileBuf(pRightFileBuf, nRightFileBuflen, pinDevID, pinRandom, &pRight, &nRightLen);
	if(ERR_SUCCESS != nRet)
	{
		if(pinDevID != NULL){
			delete[] pinDevID;
...
int GenerateDeviceID(char** ppDeviceID, int* pDeviceIDLen)
{
	...
	char szDeviceInfoLen[5] = {0};  // �洢�豸��Ϣ�����ַ���

	if(GetDiskInfo(&pDiskInfo, &nDiskInfoLen))
		return ERR_OTHER;

	sprintf_s(szDeviceInfoLen, "%04d", nDiskInfoLen); // ���͸�ʽ��Ϊ�ַ�����ʽ
	nFormateInfoLen = LENGTH_DEV_TAG + LENGTH_LEN + nDiskInfoLen;	// ��ʽ��Ӳ���豸��Ϣ�ĳ���
