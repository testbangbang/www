...
public class CropImageView extends ImageViewTouchBase {
    ...
    private boolean isBothWayStretch;
    private boolean isCropPage;
    private boolean isSplitSubScreens;
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                for (HighlightView hv : highlightViews) {
                    ...
                    if (edge != HighlightView.GROW_NONE) {
                        ...
                        lastX = event.getX();
                        lastY = event.getY();
                        motionHighlightView.setMotionEdge(edge);
                        motionHighlightView.setMode((edge == HighlightView.MOVE)
                                ? HighlightView.ModifyMode.Move
                                : HighlightView.ModifyMode.Grow);
                        if (edge == HighlightView.MOVE_SPLIT_POINT) {
                            motionHighlightView.setSplitPoint(lastX, lastY);
                        }
                        break;
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (motionHighlightView != null) {
                    // ToDo: avoid the animation refresh on E-ink Screen,cancel center Image behavior.
            ...
            case MotionEvent.ACTION_MOVE:
                if (motionHighlightView != null) {
                    motionHighlightView.handleMotion(motionEdge, event.getX(), event.getY(),
                            event.getX() - lastX, event.getY() - lastY,isBothWayStretch);
                    lastX = event.getX();
                    lastY = event.getY();
    ...
    public void ensureVisible(HighlightView hv) {
        Rect r = hv.drawRect;
    ...
    public void setBothWayStretch(boolean isBothWayStretch) {
        this.isBothWayStretch = isBothWayStretch;
    }

    public void setCropPage(boolean cropPage) {
        isCropPage = cropPage;
    }

    public void setSplitSubScreens(boolean splitSubScreens) {
        isSplitSubScreens = splitSubScreens;
    }
}
